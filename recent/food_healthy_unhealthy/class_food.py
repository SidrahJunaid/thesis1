        ...
        for line in fileinput.input(text, inplace=False):
            ...
            if final_result['healthy'] != 0 or final_result['unhealthy'] != 0:
                ...
                if final_result['compound']==0:
                      print('{},final:{}'.format(line,dict(final_result)),file=open("ex\\food_tweets.txt", "a"))
                        # print('{},final:{}'.format(line, dict(final_result)))

                if final_result['compound']>0:
                #     #
                     print(line,file=open("ex\\positive_food_sentiment.txt", "a"))
                #       print(line)
                #
                if final_result['compound']<0:
                    print(line, file=open("ex\\negative_food_sentiment.txt", "a"))
                #
                else:
